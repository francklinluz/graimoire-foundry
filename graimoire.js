...
Hooks.once('init', () => {
  game.settings.register(MODULE_ID, 'serverUrl', {
    ...
    hint: 'URL do backend Graimoire (ex: https://graimoire-production.up.railway.app)',
    scope: 'world',
    config: true,
    type: String,
  ...
  game.settings.register(MODULE_ID, 'showToAll', {
    ...
    hint: 'Se ativado, respostas aparecem para todos os jogadores. Se desativado, só para quem perguntou.',
    scope: 'world',
    config: true,
    type: Boolean,
    default: true,
  ...
  // Stores a JSON array of user IDs allowed to use /graimoire (Team plan)
  // GM always has access regardless of this setting
...
Hooks.once('ready', () => {
  ...
  if (game.user.isGM) {
    ...
    // Add player access management button to module settings
...
function userHasAccess(user) {
  ...
  // Check if player is in the allowed list (Team plan)
  try {
...
function openPlayerAccessManager() {
  ...
  const content = `
    <div style="padding:8px 0">
      ...
      ${players.length === 0
        ? '<p style="font-size:13px;opacity:0.5;font-style:italic">Nenhum jogador conectado no momento.</p>'
        : playerRows
      }
    </div>`;
...
function addPlayerAccessButton() {
  // Add button to module settings via hook when settings are rendered
...
async function handleGraimoireQuery(question, chatData) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) { showLoginPrompt(serverUrl); return; }
      if (response.status === 403 && err.limitReached) { showLimitReached(err.error, showToAll); return; }
...
function removeTyping(typingId) {
  const msg = game.messages.find(m => m.getFlag(MODULE_ID, 'typingId') === typingId);
  if (msg) msg.delete();
}
...
async function showAccessDenied() {
  ...
  const content = `
    <div class="graimoire-message graimoire-error">
      ...
      <div class="graimoire-body">
        Apenas o Mestre pode usar /graimoire no plano atual.<br>
        O plano Team libera acesso para todos os jogadores da mesa.<br><br>
...
function showLoginPrompt(serverUrl) {
  new Dialog({
    ...
    `,
    buttons: {
      login: { label: 'Abrir Graimoire', callback: () => window.open(serverUrl, '_blank') },
      cancel: { label: 'Cancelar' },
    },
    default: 'login',
  }).render(true);
